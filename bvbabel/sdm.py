...
def read_sdm(filename):
    ...
    -------
    header : dictionary
        Single subjects design matrix (SDM) header. Also used for storing motion estimates (*_3DMC.sdm).
    ...
    Description
    -------
    An SDM file consists of the following header fields: 'FileVersion', 'NrOfPredictors', 'NrOfDataPoints', 
    'IncludesConstant', 'FirstConfoundPredictor'.
    Data are columns: NrOfDataPoints x NrOfPredictors weights (float); each predictor/regressor column is preceded by a triple of RGB color values,
    each between 0-255, saved as header["Colors"], and names, saved as header["PredictorNames"]. 
    Current SDM file version: 1
    """
    ...
    for r, line in enumerate(lines[header_rows:]):
        ...
        for v in range(len(values)):
            p = values[v]
            if len(p) > 0:
                sign = p.rfind("-", 1) # detect concatenated numbers in *_3DMC.sdm files, like '0.0310625-0.000387509'
                if sign > -1:
                    if p.rfind("e") > -1: # detect scientific notation in *_3DMC.sdm files, like '5.806e-05'
